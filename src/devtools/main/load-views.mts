...
import { fixCSSHostContext } from './hack.mts';
import { ReloadEvent, WorkerBootstrapEvent } from './helpers.mts';
import * as Root from 'chrome-devtools-frontend/front_end/core/root/root.ts';
import { search, state } from './state.mts';
    ...
    async loadView() {
      return class Engine262Document extends UI.Widget.Widget {
        ...
        constructor() {
          ...
          dom.querySelector('.page-title span')!.textContent = S.engine262(L.engine262.title);
          ...
          async function render() {
            ...
            if (!(dom instanceof DocumentFragment)) throw new Error('');
            e.querySelector('article')!.innerHTML = marked(text);
          }
          render();
        }
      }.new();
...
export function modifyConsoleView() {
  function modifyUI(ConsolePanel: typeof import('chrome-devtools-frontend/front_end/panels/console/console.ts').ConsolePanel) {
    ...
    const editor = Console.element.querySelector('devtools-text-editor')!;
    editor.dispatch({
      changes: [{ from: 0, insert: state.code! }],
    });
    ...
    `;
      ...
      reloadButton.variant = Variant.TOOLBAR;
      ...
      selectFileButton.variant = Variant.TOOLBAR;
      ...
      shareButton.variant = Variant.TOOLBAR;
    ...
    addEvaluateModeOption(ConsolePanel);
    return true;
  }

  function addEvaluateModeOption(ConsolePanel: typeof import('chrome-devtools-frontend/front_end/panels/console/console.ts').ConsolePanel) {
    // eval mode
  ...
  return {
    async run() {
      const { ConsolePanel } = await import('chrome-devtools-frontend/front_end/panels/console/console.ts');
      const Console = ConsolePanel.ConsolePanel.instance();
